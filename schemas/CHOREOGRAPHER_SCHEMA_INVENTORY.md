...
| `AuditResult` | `TypedDict` | `passed: bool`; `warnings: list[str]`; `errors: list[str]`; `recommendations: list[str]` | — | **Schema pending** for operationalization outputs. |
...
**Pending schema work:** author `schemas/dereck_beach/audit_result.schema.json` to
formalize `AuditResult` before `OperationalizationAuditor` outputs can be
validated.
...
| `SemanticAnalyzer.extract_semantic_cube` output | nested `dict` | `dimensions` (`value_chain_links`, `policy_domains`, `cross_cutting_themes`); `measures` (`semantic_density`, `coherence_scores`, `overall_coherence`, `semantic_complexity`); `metadata` (`extraction_timestamp`, `total_segments`, `processing_parameters`) | — | **Schema needed** (`semantic_cube.schema.json`). |
| `PerformanceAnalyzer.analyze_performance` output | nested `dict` | Throughput metrics, bottlenecks, loss functions, recommendations keyed by value chain link | — | **Schema needed** (`performance_analysis.schema.json`). |

**Action:** Create `schemas/analyzer_one/semantic_cube.schema.json` and
`schemas/analyzer_one/performance_analysis.schema.json` before choreographer can
validate this producer.
...
| `EvidenceBundle` | `dataclass` | `dimension: CausalDimension`, `category: str`, `matches: list[str]`, `confidence: float`, `context_windows: list[str]`, `match_positions: list[int]` | — | **Schema pending** for exported evidence bundles. |

**Action:** Author `schemas/policy_processor/evidence_bundle.schema.json` (and
optional `processor_config` schema if configs are externalized).
...
| `SemanticProcessor.chunk_text` output | list[`dict`] | Each chunk: `text`, `section_type: PDMSection`, `section_id`, `token_count`, `position`, `has_table: bool`, `has_numerical: bool`, `embedding: np.ndarray` | — | Forms the base for dimension scoring; schema pending. |
| `PolicyDocumentAnalyzer.analyze` output | `dict` | `summary` (`total_chunks`, `sections_detected`, `has_tables`, `has_numerical`); `causal_dimensions` (`total_chunks`, `mean_similarity`, `max_similarity`, Bayesian evidence fields), `key_excerpts` (top chunks per dimension with content + metadata) | — | **Schema pending** (e.g., `semantic_chunking_policy/analysis_result.schema.json`). |

**Action:** Add schemas for chunk records and the aggregated analysis result so
`choreographer` can validate Bayesian evidence integration outputs.
...
| `CounterfactualScenario` | `dataclass` | `intervention: dict[str,float]`, `predicted_outcomes: dict[str,tuple[float,float,float]]`, `probability_improvement: dict[str,float]`, `narrative: str` | — | Counterfactual narratives (schema pending). |
| `ExtractedTable` | `dataclass` | `df: pd.DataFrame`, `page_number: int`, `table_type: str | None`, `extraction_method`, `confidence_score: float`, `is_fragmented: bool`, `continuation_of: int | None` | — | Tabular artifact; consider Parquet export or JSON orientation. |
...
| `ResponsibleEntity` | `dataclass` | `name`, `entity_type`, `specificity_score: float`, `mentioned_count: int`, `associated_programs: list[str]`, `associated_indicators: list[str]`, `budget_allocated: Decimal | None` | — | Schema optional. |
| `QualityScore` | `dataclass` | `overall_score`, `financial_feasibility`, `indicator_quality`, `responsibility_clarity`, `temporal_consistency`, `pdet_alignment`, `causal_coherence`, `confidence_interval: tuple[float,float]`, `evidence: dict[str,Any]` | — | Aggregated scoring; schema pending if exported. |

**Action:** Only counterfactual, table, responsible-entity, and quality score
schemas remain optional; core DAG and effect artifacts already validated.
...
| `BayesianEvaluation` | `TypedDict` | `point_estimate: float`, `credible_interval_95: tuple[float,float]`, `posterior_samples: np.ndarray`, `evidence_strength: Literal['weak','moderate','strong','very_strong']`, `numerical_coherence: float` | — | **Schema pending** for Bayesian scoring. |
...
**Action:** Provide `bayesian_evaluation.schema.json` to close the evidence
loop; configs can remain code-only unless externalized.

---

## Summary of Pending Schemas

| Module | Artifact(s) | Suggested Schema Path |
|--------|-------------|-----------------------|
| `dereck_beach.py` | `AuditResult` | `schemas/dereck_beach/audit_result.schema.json` |
| `Analyzer_one.py` | `semantic_cube`, `performance_analysis` | `schemas/analyzer_one/semantic_cube.schema.json` / `schemas/analyzer_one/performance_analysis.schema.json` |
| `policy_processor.py` | `EvidenceBundle` (and optionally `ProcessorConfig`) | `schemas/policy_processor/evidence_bundle.schema.json` |
| `semantic_chunking_policy.py` | Chunk record, analyzer result | `schemas/semantic_chunking_policy/chunk.schema.json`, `schemas/semantic_chunking_policy/analysis_result.schema.json` |
| `financiero_viabilidad_tablas.py` | `CounterfactualScenario`, `ExtractedTable`, `ResponsibleEntity`, `QualityScore` | `schemas/financiero_viabilidad/counterfactual_scenario.schema.json`, etc. |
| `embedding_policy.py` | `BayesianEvaluation` | `schemas/embedding_policy/bayesian_evaluation.schema.json` |

Choreographer should reference this table when resolving producer artifacts;
schema creation teams can treat it as the authoritative backlog for Commit 3
follow-up work.
