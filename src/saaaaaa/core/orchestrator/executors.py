...
class AdvancedDataFlowExecutor(ABC, MethodSequenceValidatingMixin):
    ...
    def __init__(self, method_executor, signal_registry=None, config: ExecutorConfig | None = None) -> None:
        ...
        if self.config is None:
            raise ValueError("ExecutorConfig must not be None")
        
        # Log config hash for traceability
        config_hash = self.config.compute_hash()
        logger.info(f"Executor initialized with config hash: {config_hash}")
    ...
    def execute_with_optimization(self, doc, method_executor,
        ...
        try:
            ...
            for idx, (class_name, method_name) in enumerate(method_sequence):
                ...
                # Execute with retry logic using config
                method_start = time.time()
                success = False
                max_retries = self.config.retry + 1  # retry=2 means 3 total attempts (1 initial + 2 retries)
                prepared_kwargs = {}  # Initialize to prevent UnboundLocalError in failure logging
                
                # Setup timeout handler using signal.alarm
                def timeout_handler(signum, frame):
                    raise TimeoutError(f"Method {method_key} exceeded timeout of {self.config.timeout_s}s")
                
                for attempt in range(max_retries):
                    try:
                        # Set timeout alarm (Unix-only, will be no-op on Windows)
                        try:
                            old_handler = signal.signal(signal.SIGALRM, timeout_handler)
                            signal.alarm(int(self.config.timeout_s))
