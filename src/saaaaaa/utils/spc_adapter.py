...
This adapter converts Smart Policy Chunks (SPC) documents from the ingestion
...
structlog = try_import("structlog", required=False, hint="Structured logging for SPCAdapter")
pyarrow = try_import("pyarrow", required=False, hint="Arrow serialization for SPCAdapter")
...
class SPCAdapterError(Exception):
    """Raised when SPC adaptation fails."""
...
class SPCAdapter:
    """
    Adapter for converting Smart Policy Chunks to PreprocessedDocument.
    ...
    def __init__(self) -> None:
        """Initialize SPC adapter."""
        self._conversions_count = 0
        if structlog is not None:
            logger.info("spc_adapter_initialized")
        else:
            logger.info("spc_adapter_initialized")
    ...
    def to_preprocessed_document(
        ...
        *,
        ...
        """
        Convert Smart Policy Chunks to PreprocessedDocument.
        
        Args:
            cpp: Smart Policy Chunks package from ingestion pipeline (parameter name kept for compatibility)
        ...
        Raises:
            SPCAdapterError: If conversion fails with prescriptive error message
        """
        # Validate input
        if not cpp:
            raise SPCAdapterError(
                "Cannot convert None SPC to PreprocessedDocument",
                details={"input_type": type(cpp).__name__}
            )
        
        if not cpp.chunk_graph or not cpp.chunk_graph.chunks:
            raise SPCAdapterError(
                "SPC chunk graph is empty. Ensure ingestion pipeline completed successfully.",
                details={
        ...
        if document_id is None:
            ...
            document_id = (
                chunks[0].id.split("_")[0] if chunks else "spc_document"
            )
        ...
        if structlog is not None:
            logger.info(
                "spc_to_preprocessed_document_complete",
            ...
            logger.info(
                f"spc_to_preprocessed_document_complete: document_id={document_id}, "
        ...
        if provenance_completeness < 1.0:
            if structlog is not None:
                ...
                logger.warning(
                    f"spc_incomplete_provenance: document_id={document_id}, "
...
def adapt_spc_to_orchestrator(
    ...
    *,
    ...
    Args:
        cpp: Smart Policy Chunks package (parameter name kept for compatibility)
